...
class BaoCaoTienDo(models.Model):
    ...
    nhan_vien_id = fields.Many2one('nhan_vien', string="Người phụ trách", required=True)   
    ...
    tien_do = fields.Integer(string="Tiến độ (%)", required=True, digits=(3, 0), help="Đánh giá phần trăm hoàn thành của công việc")
    ...
    def _onchange_cong_viec_id(self):
        if self.cong_viec_id and self.cong_viec_id.nguoi_phu_trach_id:
            self.nhan_vien_id = self.cong_viec_id.nguoi_phu_trach_id.id